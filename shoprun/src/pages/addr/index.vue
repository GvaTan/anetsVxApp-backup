...
<template>
	<view class="page">
		<view class="addr">
            <text>收货人:</text><input type="text"/>
        </view>
        <view class="addr">
            <text>手机号码:</text><input  type="text"/>
        </view>
		
		<!-- 不固定默认值 -->
		<view class="row">
			<text class="name">省/市/区：</text>
			<view class="value">
				<biaofun-region :defaultValue="defaultValue" @change="onChangeRegion"></biaofun-region>
			</view>
			<text class="iconfont right-icon" style="height:20upx;line-height:20upx">></text>
		</view>

        <view class="addr">
            <text>街道地址:</text><input placeholder="请填写具体的地址" type="text"/>
        </view>
...
export default {
	...
	async onReady() {
		...
		let timer = setTimeout(() => {
			_this.defaultValue = ['山东省', '济南市', '槐荫区'];
			clearTimeout(timer);
		}, 3000);
	},
